...
export function HeroSectionV2({ user, tradeMode, setTradeMode }: HeroSectionV2Props) {
    ...
    if (saved) {
      try {
        ...
        if (parsed.initialText) {
          ...
          // Only auto-open if they were in the middle of auth
          // This allows the flow to resume and finish after login redirect
          if (parsed.step === "auth") {
  ...
  return (
    <>
      <section id="onseek-hero" className="relative w-full h-screen bg-[#6925DC] overflow-hidden">
        <div className="mx-auto w-full max-w-[1360px] h-full flex flex-col lg:flex-row items-stretch px-8 sm:px-16 lg:px-6">
          {/* Content Side */}
          <div className="flex-[0.55] flex flex-col justify-center items-start py-12 lg:py-16 pt-28 lg:pt-36 relative z-10">
            {/* Main Title */}
            <h1
              className="text-white text-[42px] sm:text-[68px] leading-[1.05] tracking-tight mb-10 font-black max-w-3xl"
              ...
              <div className="w-full max-w-xl relative group">
                <div className="relative flex flex-col w-full min-h-[100px] h-auto bg-white rounded-[24px] px-8 pt-6 pb-14 transition-all duration-300 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.15)] hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.2)]">
                  <textarea
                    ...
                    rows={2}
                    className="bg-transparent border-none outline-none w-full text-[#1A1A1A] placeholder:text-gray-500 placeholder:text-[17px] text-[17px] font-medium resize-none overflow-hidden"
                  />
        ...
        <div className="hidden lg:flex flex-[0.45] relative items-center justify-center self-stretch pt-24">
          <div className="relative h-full max-h-[612px] aspect-square rounded-[24px] overflow-hidden shadow-2xl">
            <Image
          ...
          {/* Poppy Pink Cloud Overlay - Temporarily Hidden */}
